...
def sample(args, device, generative_model, target_data, dataset_info,
    ...
    if args.probabilistic_model == 'diffusion':
        x, h = generative_model.sample(batch_size, max_n_nodes, target_data, node_mask, edge_mask, context, fix_noise=fix_noise)
...
def sample_sweep_conditional(args, device, generative_model, data, dataset_info, prop_dist, n_nodes=19, n_frames=100):
    ...
    one_hot, charges, x, node_mask = sample(args, device, generative_model, data, dataset_info, prop_dist, nodesxsample=nodesxsample, context=context, fix_noise=True)
    return one_hot, charges, x, node_mask
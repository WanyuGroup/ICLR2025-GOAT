...
def split_dataset(data, split_idxs):
    ...
    split_idxs :  dict
        ...
        values are the keys for the items in mol_data that go into the split.
...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True):
    """
    Take a set of datafiles and apply a predefined mol_data processing script to each
    ...
    stack : bool, optional
        ?????
    """
    logging.info('Processing mol_data file: {}'.format(data))
...
def process_xyz_md17(datafile):
    ...
    datafile : python file object
        File object containing the molecular mol_data in the MD17 dataset.

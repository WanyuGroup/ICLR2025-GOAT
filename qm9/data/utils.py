...
def initialize_datasets(args, datadir, dataset, subset=None, splits=None,
    ...
    datadir : str
        Path to the directory where the mol_data and calculations and is, or will be, stored.
    dataset : str
    ...
    # header = ['alpha', 'gap', 'homo', 'lumo', 'mu', 'Cv']
    # for data_text, dataset in datasets.items():
    #     combined_tensor = torch.stack((dataset['alpha'], dataset['gap'], dataset['homo'], dataset['lumo'], dataset['Cv'], dataset['mu']), dim=1)
    #     # Define the file path to save the CSV file
    #     file_path = f'{data_text}_property.csv'
    #     # Convert the tensor to a NumPy array for CSV writing
    #     combined_array = combined_tensor.numpy()
    ...
    if dataset == 'qm9_scaffold_s':
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/Scaffold_QM9/QM9_s3_scaffold_split_{splits_name}_dataset.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif dataset == 'qm9_noise':
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/Scaffold_QM9/QM9_Noise_{splits_name}.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif 'qm9_reflow' in dataset:
        for splits_name in ['train', 'test', 'valid']:
            with open(f'reflow/{dataset}_{splits_name}.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
        if datasets['train']['charges'].size(0) > 100000:
            datasets['train'] = {key: datasets['train'][key][:100000] for key in datasets['train'].keys()}
    elif dataset == 'qm9_scaffold_ring':
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/Scaffold_QM9/QM9_ring_scaffold_split_{splits_name}_dataset.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif dataset == 'qm9_scaffold_target':
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/Scaffold_QM9/QM9_scaffold_target_{splits_name}_dataset.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif 'qm9_ring' in dataset:
        with open(f'../../Data/Scaffold_QM9/QM9_ring_number_{dataset[-1]}_dataset.pickle', 'rb') as file:
            datasets[dataset[-1]] = pickle.load(file)
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/Scaffold_QM9/QM9_ring_scaffold_split_{splits_name}_dataset.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif dataset == 'qm9_second_half_positive':
        # Load the dictionary from the pickle file
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/split_QM9/qm9_{splits_name}_{property}_positive.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif dataset == 'qm9_second_half_negative':
        # Load the dictionary from the pickle file
        for splits_name in ['train', 'test', 'valid']:
            with open(f'../../Data/split_QM9/qm9_{splits_name}_{property}_negative.pickle', 'rb') as file:
                datasets[splits_name] = pickle.load(file)
    elif dataset != 'qm9':
        np.random.seed(42)
        ...
        elif dataset == 'qm9_few_shot':
            half_len = len(datasets['train']['num_atoms'])//2
            sliced_perm = fixed_perm[half_len:half_len + args.target_data_size]
        else:
            raise Exception('Wrong dataset name')
        for key in datasets['train']:
            datasets['train'][key] = datasets['train'][key][sliced_perm]
    if 'reflow' in dataset:
        ignore_check_species = True
    else:
        ignore_check_species = False
    ...
    all_species = _get_species(datasets, ignore_check=ignore_check_species)

    # Now initialize MolecularDataset based upon loaded mol_data
...
def _get_species(datasets, ignore_check=False):
    ...
    all_species : Pytorch tensor
        List of all species present in the mol_data.  Species labels shoguld be integers.

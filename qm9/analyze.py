...
def main_check_stability(remove_h: bool, batch_size=32):
    from utils import datasets_config
    import qm9.dataset as dataset
...
def filer_molecules(molecule_list, dataset_info):
    one_hot = molecule_list['one_hot']
    x = molecule_list['positions']
    node_mask = molecule_list['atom_mask']
    ...
    stable_index_list = []
    for index, mol in enumerate(processed_list):
        pos, atom_type = mol
    ...
    metrics = BasicMolecularMetrics(dataset_info)
    valid, valid_list = metrics.filter(processed_list)
    success_list = list(set(valid_list).intersection(set(stable_index_list)))
    molecule_list = {key: molecule_list[key][success_list] for key in molecule_list}

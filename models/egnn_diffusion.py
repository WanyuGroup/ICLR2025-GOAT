...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
            self,
            dynamics: EGNN_dynamics, in_node_nf: int, n_dims: int,
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        h_cat = z0[:, :, self.n_dims:-1]  if self.include_charges else z0[:, :, self.n_dims:]
        x, h_cat, h_int = self.unnormalize(x, h_cat, h_int, node_mask)
    ...
    def log_pxh_given_z0_without_constants(
        ...
        # Compute sigma_0 and rescale to the integer scale of the mol_data.
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        uf.assert_mean_zero_with_mask(z_t[:, :, :self.n_dims], node_mask)

        # Neural net prediction.
        # z_t batchsize, nodesize, 7
        # t batchsize, 1
        # node_mask batchsize, nodesize, 1
        net_out = self.phi(z_t, t, node_mask, edge_mask, context)
        # net_out batchsize, nodesize, 7
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False):
        ...
        # Compute mu for p(zs | zt).
        uf.assert_mean_zero_with_mask(zt[:, :, :self.n_dims], node_mask)
        uf.assert_mean_zero_with_mask(eps_t[:, :, :self.n_dims], node_mask)
        ...
        zs = torch.cat(
            [uf.remove_mean_with_mask(zs[:, :, :self.n_dims],
                                                   node_mask),
